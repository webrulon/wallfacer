...
// GenerateOversight asynchronously produces a high-level aggregated summary of a
// task's agent execution trace and persists it as oversight.json in the task
// directory. It is intended to be launched as a goroutine when a task
// transitions to waiting, done, or failed.
//
// The summary is generated by running a lightweight Claude Code container that
// receives a pre-processed compact activity log extracted from the raw NDJSON
// turn output files. Claude groups the activities into logical phases and
...
// ndjsonLine is used to parse individual NDJSON messages from Claude Code output.
...
// oversightResult is the JSON structure Claude produces.
...
// runOversightAgent runs a lightweight Claude container with the activity log and
